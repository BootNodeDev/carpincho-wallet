...
This is the canonical agent configuration for the `carpincho-wallet` repository — the CIP-0103 Canton wallet, extracted from the `cn-dappbooster` monorepo into its own repo. `CLAUDE.md` is a compatibility shim that points here.
...
| Linting | Biome | `biome.json` + `@biomejs/biome`; this project's rules (`@/` alias enforcement, Tailwind CSS parsing) live in a path `override` scoped to `src/` and `test/`. Enforced via the pre-commit hook; run `npm run lint` manually |
...
- Build outputs: `dist/` for the web build and `dist-extension/` for the Chrome extension (`npm run build:extension`)
