...
Carpincho is a wallet/provider UI with an encrypted local vault, an injected CIP-0103 browser provider, and optional WalletConnect support. A dApp connects to Carpincho through the injected provider by default; Carpincho answers wallet/provider requests and runs Canton work against the participant a Splice Wallet Gateway names. A dApp must be explicitly approved per origin before Carpincho exposes accounts or signs for it: an unapproved site sees no accounts, and the first `connect` opens an approval prompt in Carpincho.

```text
dApp frontend -> injected CIP-0103 provider -> carpincho-wallet -> Canton participant
                                                        \-> Splice Wallet Gateway (discovery + token)
```
