...
- **Shared primitives that don't spread props** expose an explicit `testId` prop forwarded to their root DOM node — `Sheet`, `Select`, `TabTrigger`, `MenuRow`, `FileDropInput`, `Collapsible`, `Copyable`, `Alert`, `DangerConfirm` (`testId` / `confirmTestId`), `DetailRow` (`copyTestId`), `AmountField`, `ConfirmPasswordForm` (`passwordTestId` / `submitTestId`). When you build a new primitive that hides its DOM element, add the same `testId` passthrough so callers can label it instead of reaching past the abstraction.
...
Accounts are scoped to the network that endpoint reports. A Canton party is hosted on the one network it was created on, so `src/vault/networkScope.ts` filters the vault down to the network in use (polled once by `NetworkProvider` in `src/network/`, read through `useNetwork`) before anything sees it: `useVault().accounts` / `.primary`, the `accountsChanged` payload, the extension snapshot, and the dApp-facing `listAccounts` / `getPrimaryAccount`. `addAccount` stamps that network itself, so no caller passes one. `useVault().offNetworkCount` is how many accounts the vault holds elsewhere, so a screen can account for them; a vault whose accounts are all elsewhere routes to `AddNetworkAccount` instead of Home. An endpoint that reports no network id (unreachable) scopes nothing, and the whole vault stays visible.
