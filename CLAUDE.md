...
Accounts are scoped to the network that endpoint reports. A Canton party is hosted on the one network it was created on, so `src/vault/networkScope.ts` filters the vault down to the network in use (polled once by `NetworkProvider` in `src/network/`, read through `useNetwork`) before anything sees it: `useVault().accounts` / `.primary`, the `accountsChanged` payload, the extension snapshot, and the dApp-facing `listAccounts` / `getPrimaryAccount`. `addAccount` stamps that network itself, so no caller passes one. `useVault().offNetworkCount` is how many accounts the vault holds elsewhere; routing reads it so a vault whose accounts are all elsewhere goes to `AddNetworkAccount` instead of Home, and no screen mentions those accounts. An endpoint that reports no network id (unreachable) scopes nothing, and the whole vault stays visible.
