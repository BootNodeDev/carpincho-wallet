...
Accounts are scoped to the network that endpoint reports. A Canton party is hosted on the one network it was created on, so `src/vault/networkScope.ts` filters the vault down to the network in use (polled once by `NetworkProvider` in `src/network/`, read through `useNetwork`) before anything sees it: `useVault().accounts` / `.primary`, the `accountsChanged` payload, the extension snapshot, and the dApp-facing `listAccounts` / `getPrimaryAccount`. `useVault().offNetworkCount` is how many accounts the vault holds elsewhere, so a screen can account for them. An endpoint that reports no network id (unreachable) scopes nothing, and the whole vault stays visible.
