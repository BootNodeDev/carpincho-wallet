...
| Linting | Biome | `biome.json` + `@biomejs/biome`; this project's rules (`@/` alias enforcement, Tailwind CSS parsing) live in a path `override` scoped to `src/` and `test/`. Enforced via the pre-commit hook; run `pnpm run lint` manually |
...
- `pnpm run lint`
- `pnpm test`
- `pnpm run build` (when feasible for runtime-impacting changes)
