...
- **`contentScript.ts`** — Injected into every matched page; announces Carpincho with `canton:announceProvider`, responds to `canton:requestProvider`, and bridges page `window.postMessage` requests to the extension runtime. It also answers `SPLICE_WALLET_EXT_READY` with an ack and turns the `SPLICE_WALLET_EXT_OPEN` that `sdk.open()` posts into a `CARPINCHO_OPEN_WALLET` for the background. The announced `icon` is a base64 `data:image/png` URI inlined from `public/icons/carpincho-48.png` at build time through the `__WALLET_ICON_DATA_URL__` Vite define: the dApp SDK's wallet picker renders in a `blob:` document, so a `chrome-extension://` URL would render broken.
