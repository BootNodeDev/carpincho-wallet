...
- **`background.ts`** — Persistent service worker. Maintains a queue of pending dApp requests, updates the action badge with the count, and caches a wallet snapshot for the popup. A queued request also opens the wallet on its own, as a `chrome.windows.create` popup window: Chrome only allows `action.openPopup` from a gesture-carrying event, and the focused window when a dApp request lands is the SDK's toolbar-less picker. Only one such window exists at a time — a later request focuses it — answering the last pending request closes it, and closing it by hand answers every still-pending request with the `4001 user rejected` error so the dApp's call settles. Clicking the toolbar icon still opens the normal popup. Also records the set of direct (injected-provider) dApp origins that have connected to `chrome.storage.session`, which the popup reads and watches via `storage.session.onChanged` to drive the footer's connection state. That connected-origins set is also a security input: it decides whether a requesting origin is connected (gating account access), and wallet events are broadcast only to connected dApp tabs.
