...
src/
  ...
  vault/            Encrypted local vault: PBKDF2 key derivation, AES-GCM storage, React context,
                    account scoping to the network in use
...
- **`events.ts`** — The one list of CIP-0103 events the wallet pushes (`accountsChanged`, `connected`, `statusChanged`, `txChanged`). The WalletConnect session declares it at approval time and `broadcastWalletEvent` is typed by it, so a dApp is offered the same set however it connected.
...
There is no router. `Shell` picks one view from `useVault()` via the pure `selectShellView` helper in `src/App.tsx`, branching on `hasVault`, `isLocked`, `accounts.length` and `offNetworkCount`: no vault → `OnboardingFlow` starting at the create-vault step; unlocked vault with no account at all → `OnboardingFlow` which runs the Configure RPC step (gated on a wallet-service probe) then the create-first-account step, which presents two tabs — "Create new account" and "Restore from backup", where the restore tab reuses the dashboard `ImportVaultForm` to merge a backup's accounts under the new local vault password and never creates a Canton party; unlocked vault whose accounts are all on other networks → `AddNetworkAccount`, which asks only for a new account (a backup of this vault would import nothing: every entry is a party-and-network duplicate) and keeps the connection footer so switching endpoint is the way out; locked vault → `UnlockView`; unlocked vault with at least one account for the network in use → `HomeView`. While `useVault()` reports `isLoading`, `Shell` renders a centred spinner instead of any view so the session-restore decision lands in one paint and the Unlock screen never flashes.
